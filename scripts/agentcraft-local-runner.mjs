...
await heartbeatProjectRunnerWithRetry('initial project runner heartbeat');
...
async function startLocalRunnerWorker(job, options = {}) {
  ...
  try {
    ...
    const runtimeCommand = Array.isArray(job.command) && job.command.length
      ? job.command
      : localRunnerDefaultRuntimeCommand(agentType);
    const runArgs = [
      'run',
      '-d',
      '--name',
...
function localRunnerDefaultRuntimeCommand(agentType) {
  const normalized = String(agentType || 'hermes-agent').trim().toLowerCase().replace(/_/g, '-');
  if (normalized === 'hermes-agent' || normalized === 'hermes') return ['gateway', 'run'];
  if (['pi', 'pi-agent', 'codex', 'claude-code', 'claude'].includes(normalized)) {
    return ['node', '/opt/agentcraft/cli-adapter.mjs'];
  }
  return ['python3', '/opt/agentcraft/cli-adapter.py'];
}
  ...
  while (!shuttingDown) {
    try {
      ...
      try {
        ...
        const message = await localRuntimeRequestErrorMessage(job, error);
        await request(localRunnerPath(job.projectId, job.memberId, `requests/${encodeURIComponent(requestId)}/complete`), {
          method: 'POST',
          body: { status: 'ERROR', error: message },
        }).catch(() => null);
...
async function localRuntimeRequestErrorMessage(job, error) {
  ...
  try {
    ...
    if (status.trim()) parts.push(`container ${job.containerName}: ${status.trim()}`);
  } catch (inspectError) {
    parts.push(`container ${job.containerName}: inspect failed: ${inspectError?.message || inspectError}`);
  }
  try {
    const logs = await docker(['logs', '--tail', '80', job.containerName]);
    const sanitizedLogs = redactSensitiveText(logs.trim());
    if (sanitizedLogs) parts.push(`recent logs:\n${truncateText(sanitizedLogs, 4000)}`);
  } catch (logsError) {
    parts.push(`recent logs unavailable: ${logsError?.message || logsError}`);
  }
  return truncateText(redactSensitiveText(parts.join('\n')), 5000);
}
...
async function heartbeatProjectRunnerWithRetry(label, attempts = 5) {
  let lastError = null;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      return await heartbeatProjectRunner();
    } catch (error) {
      lastError = error;
      if (shouldStopPolling(error) || attempt >= attempts) break;
      const delayMs = Math.min(10_000, 750 * attempt);
      console.error(`${label} failed (${attempt}/${attempts}): ${error?.message || error}. Retrying in ${delayMs}ms.`);
      await sleep(delayMs);
    }
  }
  throw lastError;
...
function redactSensitiveText(text) {
  return String(text || '')
    ...
    .replace(/\beyJ[A-Za-z0-9_-]{12,}\.[A-Za-z0-9_-]{12,}\.[A-Za-z0-9_-]{12,}\b/g, '[jwt redacted]')
    .replace(/\b(sk|ak|pk)-[A-Za-z0-9_-]{16,}\b/g, '$1-[redacted]');
}
